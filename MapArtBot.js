...
class MapArtBot extends MinimalBot {
    ...
    async initialize() {
        ...
        this.stripPlacer = new StripPlacer(this.bot, this.mcData, mapArtOffsets, this.progressManager);
        ...
        this.bot.chat(`/msg ${this.master} MapArtBot connected and ready for commands.`);
    ...
    async handleCommand(message) {
        ...
        switch (command.toLowerCase()) {
            ...
            case 'status':
                ...
                break;
            default:
                this.bot.chat(`/msg ${this.master} Unknown command. Try: start, pause, continue, clear, status.`);
        }
    }

    async handleStart(imageSource) {
        if (this.state !== 'IDLE') {
            return this.bot.chat(`/msg ${this.master} A project is already active. Use 'clear' first.`);
        }
        if (!imageSource) {
            return this.bot.chat(`/msg ${this.master} Please provide an image URL or filename. Usage: start <url_or_file>`);
        }
        this.bot.chat(`/msg ${this.master} Processing image: ${imageSource}...`);
        const imageData = await ImageProcessor.processImage(imageSource);
        if (!imageData) {
        ...
        this.bot.chat(`/msg ${this.master} New map art started. Beginning construction.`);
        this.state = 'BUILDING';
    }
    ...
    handleStatus() {
        const progress = this.progressManager.get();
        if (!progress.isActive) {
            return this.bot.chat(`/msg ${this.master} No active project. Current state: ${this.state}.`);
        }
        const percentage = this.progressManager.getCompletionPercentage().toFixed(2);
        this.bot.chat(`/msg ${this.master} Status: ${this.state} | Image: ${progress.imageSource} | Strip: ${progress.currentStripIndex} | Overall Progress: ${percentage}%`);
    }
    ...
    async mainLoop() {
        while (true) {
            ...
            if (this.state === 'BUILDING') {
                const required = this.progressManager.getRequiredMaterialsForCurrentStrip();
                ...
                if (!this._hasMaterials(required)) {
                    this.bot.chat(`/msg ${this.master} Insufficient materials for current strip. Switching to restock mode.`);
            ...
            if (this.state === 'RESTOCKING') {
                ...
                if (success) {
                    this.bot.chat(`/msg ${this.master} Restocking complete. Resuming building.`);
                    this.state = 'BUILDING';
                } else {
                    this.bot.chat(`/msg ${this.master} Failed to restock all required materials. Pausing task.`);
                    this.state = 'PAUSED';
                }
            }
        }
